...
┌─────────────────────────────────────────────────────────────┐
│                     用户 API 层                               │
│  fragmentUpload / fragmentUpload1                            │
│  - 选择器模式（事件监听）                                     │
...
### 1. API 层 (`fragmentUpload.ts` / `fragmentUpload1.ts`)
...
- **模式区分**：
  - `fragmentUpload`：批量回调模式，所有分片处理完成后回调
  - `fragmentUpload1`：立即回调模式，每个分片处理完成后立即回调

### 2. 文件分片层 (`fragmentFile.ts` / `fragmentFile1.ts`)
...
### 批量回调模式 (`fragmentUpload`)

```
用户调用 fragmentUpload(file, options)
    ↓
...
### 立即回调模式 (`fragmentUpload1`)

```
用户调用 fragmentUpload1(file, options)
    ↓
文件验证
    ↓
创建取消控制器
...
**解决方案**：
- `fragmentUpload`：批量回调，适合需要完整文件信息
- `fragmentUpload1`：立即回调，适合实时上传场景
